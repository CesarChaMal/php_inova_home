...
FROM 
    ...
    FROM 
     ...
     AND  TIME(RIGHT(TRIM(Dialed),8)) BETWEEN ".$horas[$i].") 
     AS llamadas_completadas,
     (SELECT SUM(Duration) AS total FROM ".$tabla[$a]." WHERE YEAR(Dialed)='".$anio."' AND MONTH(Dialed)='".$mes."' AND DAYOFMONTH(Dialed)='".$dia."'
     AND  TIME(RIGHT(TRIM(Dialed),8)) BETWEEN ".$horas[$i].") 
     AS segundos,
...
if($_GET['ajax']==7){  
    ...
    $initialport=$_POST['puertoinicial'];
...
function ocupacion($fechainicio,$fechafinal,$tablename,$cantidad,$initialport,$finalport,&$sql){
    $i=0;
    $fechainiciotemp = $fechainicio;
    $fechafinaltemp = $fechafinal;
    $accumulated_minutes = 0;
    $portinitemp = $initialport;
    $portfintemp = $finalport;
    
    $rs = new ResultSet();
    $portmin = "SELECT MIN(LocalChannel) INTO @portmin FROM $tablename";
    $portmin = $GLOBALS["conexion"]->executeQuery($portmin);
    $portmin = "SELECT @portmin";
    $portmin = $GLOBALS["conexion"]->executeQuery($portmin);
    $portmin = $portmin->get("@portmin");
    
    $portmax = "SELECT @portmax:= MAX(LocalChannel) FROM $tablename";
    $portmax = $GLOBALS["conexion"]->executeQuery($portmax);
    $portmax = "SELECT @portmax";
    $portmax = $GLOBALS["conexion"]->executeQuery($portmax);
    $portmax = $portmax->get("@portmax");
    
    if ($portfintemp > $portmax)
       $portfintemp = $portmax;

    if ($portinitemp < $portmin)
       $portfintemp = $portmin;
...
//    $acumulated=$d1;
    ...
    list($year,$month,$day)= explode("-",$fechainicio);
    $fechainiciotemp = $fechainicio;

    list($year,$month,$day)= explode("-",$fechainiciotemp);
    $fechafinaltemp = mktime(0,0,0,$month,$day,$year);
    $acumulated1=$fechafinaltemp;
    $acumulated2=$fechafinaltemp+86400;
    $fechafinaltemp = date('Y-m-d',$fechafinaltemp+86400);
    $dates = array();
    for ($i=0;$i<$Difference_Between_days;$i++){
        $dates[$i]=date('Y-m-d',$acumulated1);
        ...
        $acumulated1+=86400;
        $acumulated2+=86400;
        if ($i==($Difference_Between_days-1))
            $sql .= "FORMAT(final.porcentaje".($i+1).",4) AS '".$dates[$i]."'";
        else
            $sql .= "FORMAT(final.porcentaje".($i+1).",4) AS '".$dates[$i]."',";
    }
    ...
    for ($i=0;$i<$Difference_Between_days;$i++){
        ...
        $acumulated2+=86400;
        if ($i==($Difference_Between_days-1))
            $sql .= "FORMAT(final.porcentaje".($i+1).",4) )";
        else
            $sql .= "FORMAT(final.porcentaje".($i+1).",4) + ";
    }

    $sql .= " AS Porcentaje_Total,";
    $sql .= "final.acumulado AS Total_x_Dias,";
    $sql .= "final.total as Maximo_Minutos,";
    $sql .= "final.disponibles AS Minutos_Disponibles,";
    $sql .= "final.sobrantes AS Minutos_Sobrantes ";
    $sql .= "FROM( ";
    $sql .= "SELECT ";
    ...
    //$total_percent = 0;
    $total_percent = $GLOBALS["conexion"]->executeQuery("SET @total_percent = 0;");
    //$left_percent = 100;
    $left_percent = $GLOBALS["conexion"]->executeQuery("SET @left_percent = 100;");
    $left_percent = $GLOBALS["conexion"]->executeQuery("SELECT @left_percent;");
    $left_percent = $left_percent->get("@left_percent");
    //$percent_per_day = 0;
    $percent_per_day = $GLOBALS["conexion"]->executeQuery("SET @percent_per_day = 0;");
    ...
    for ($i=0;$i<$Difference_Between_days;$i++){
        ...
        if ($i==($Difference_Between_days-1)){
            $sql2 = "";
            $sql2 .= "SELECT @percent_per_day:= (dias.dia/dias.total)*100 AS porcentaje FROM";
            $sql2 .= "(SELECT minutos.total AS dia,total.cantidad AS total FROM ";
            $sql2 .= "(SELECT IF(SUM(CEILING(Duration/60)) IS NULL,0,SUM(CEILING(Duration/60))) AS total FROM ".$tablename;
            $sql2 .= " WHERE CONCAT(DATE(TRIM(Dialed)),' ', TIME(RIGHT(TRIM(Dialed),8))) >= '".$dates[$i];
            $sql2 .= "' AND CONCAT(DATE(TRIM(Dialed)),' ', TIME(RIGHT(TRIM(Dialed),8))) < '".$fechafinal;
            $sql2 .= "')AS minutos";
            $sql2 .= ",(SELECT ".$cantidad." as cantidad) AS total";
            $sql2 .= ") AS dias";
        }else{
            $sql2 = "";
            $sql2 .= "SELECT @percent_per_day:= (dias.dia/dias.total)*100 AS porcentaje FROM";
            ...
            $sql2 .= "(SELECT IF(SUM(CEILING(Duration/60)) IS NULL,0,SUM(CEILING(Duration/60))) AS total FROM ".$tablename;
            $sql2 .= " WHERE CONCAT(DATE(TRIM(Dialed)),' ', TIME(RIGHT(TRIM(Dialed),8))) >= '".$dates[$i];
            $sql2 .= "' AND CONCAT(DATE(TRIM(Dialed)),' ', TIME(RIGHT(TRIM(Dialed),8))) < '".$dates[$i+1];
            $sql2 .= "')AS minutos";
            $sql2 .= ",(SELECT ".$cantidad." as cantidad) AS total";
            $sql2 .= ") AS dias";
        }

        $percent_per_day = $GLOBALS["conexion"]->executeQuery($sql2);
        $percent_per_day = $GLOBALS["conexion"]->executeQuery("SET @percent_per_day = FORMAT(@percent_per_day,4)");
        $percent_per_day = $GLOBALS["conexion"]->executeQuery("SELECT @percent_per_day;");
        $percent_per_day = $percent_per_day->get("@percent_per_day");
        
        $total_percent = $GLOBALS["conexion"]->executeQuery("SET @total_percent = @total_percent + @percent_per_day;");
        $total_percent = $GLOBALS["conexion"]->executeQuery("SELECT @total_percent;");
        $total_percent = $total_percent->get("@total_percent");

        if ($i==($Difference_Between_days-1)){
            $sql .= "IF(suma.porcentaje".($i+1)." > ".$left_percent."";
            $sql .= ",IF(".$left_percent."<0,0,".$left_percent."),suma.porcentaje".($i+1).")";
            $sql .= " AS porcentaje".($i+1);
        }else{
            $sql .= "IF(suma.porcentaje".($i+1)." > ".$left_percent."";
            $sql .= ",IF(".$left_percent."<0,0,".$left_percent."),suma.porcentaje".($i+1).")";
            $sql .= " AS porcentaje".($i+1).",";
    ...
    $sql .= ",suma.acumulado AS acumulado,";
    $sql .= "suma.total AS total,";
    $sql .= "IF(suma.acumulado<=suma.total, suma.total-suma.acumulado, 0) AS disponibles,";
    $sql .= "IF(suma.acumulado<=suma.total, 0, abs(suma.total-suma.acumulado)) AS sobrantes ";
    $sql .= "FROM (";
    $sql .= "SELECT ";
    ...
    for ($i=0;$i<$Difference_Between_days;$i++){
        ...
        $acumulated2+=86400;
        if ($i==($Difference_Between_days-1))
            $sql .= "(dias.dia".($i+1)."/dias.total)*100 AS porcentaje".($i+1)." ";
        else
            $sql .= "(dias.dia".($i+1)."/dias.total)*100 AS porcentaje".($i+1).", ";
    }

    $sql .= ", dias.total AS total";
    $sql .= ", dias.acumulado AS acumulado FROM(";
    $sql .= "SELECT ";
    ...
    for ($i=0;$i<$Difference_Between_days;$i++){
        ...
        $acumulated2+=86400;
        if ($i==($Difference_Between_days-1))
            $sql .= "minutos".($i+1).".total AS dia".($i+1)." ";
        else
            $sql .= "minutos".($i+1).".total AS dia".($i+1).", ";
    }
    ...
    for ($i=0;$i<$Difference_Between_days;$i++){
        ...
        $acumulated2+=86400;
        if ($i==($Difference_Between_days-1))
            $sql .= "minutos".($i+1).".total ) ";
        else
            $sql .= "minutos".($i+1).".total + ";
    }

    $sql .= " AS acumulado,total.cantidad AS total FROM ";
    ...
    for ($i=0;$i<$Difference_Between_days;$i++){
        if ($i==($Difference_Between_days-1)){
            $sql .= "(SELECT IF(SUM(CEILING(Duration/60)) IS NULL,0,SUM(CEILING(Duration/60))) AS total FROM ".$tablename;
            $sql .= " WHERE CONCAT(DATE(TRIM(Dialed)),' ', TIME(RIGHT(TRIM(Dialed),8))) >= '".$dates[$i]." 00:00:00' AND ";
            $sql .= "CONCAT(DATE(TRIM(Dialed)),' ', TIME(RIGHT(TRIM(Dialed),8))) < '".$fechafinal." 00:00:00'";
            $sql .= " AND LocalChannel BETWEEN ".$portinitemp." AND ".$portfintemp.")AS minutos".($i+1);
        }else{
            $sql .= "(SELECT IF(SUM(CEILING(Duration/60)) IS NULL,0,SUM(CEILING(Duration/60))) AS total FROM ".$tablename;
            $sql .= " WHERE CONCAT(DATE(TRIM(Dialed)),' ', TIME(RIGHT(TRIM(Dialed),8))) >= '".$dates[$i]." 00:00:00' AND ";
            $sql .= "CONCAT(DATE(TRIM(Dialed)),' ', TIME(RIGHT(TRIM(Dialed),8))) < '".$dates[$i+1]." 00:00:00'";
            $sql .= " AND LocalChannel BETWEEN ".$portinitemp." AND ".$portfintemp.")AS minutos".($i+1).", ";
        }    
    }

    $sql .= ",(SELECT ".$cantidad." as cantidad) AS total";
    $sql .= ") AS dias";
    $sql .= ") AS suma";
    $sql .= ") AS final";
    
    //echo $sql;
    //$sql = $GLOBALS["conexion"]->executeQuery($sql);
    return $sql;
/*
    $dates = array();
    $acumulated=$d1;
    for ($i=0;$i<$Difference_Between_days;$i++){
        $dates[$i]=date('Y-m-d',$acumulated);
        $acumulated+=86400;
    }
*/
}

//function ocupacion2($dates,$fechainicio,$fechafinal,$tablename,$cantidad,$initialport,$finalport,$sql){
function ocupacion3($dates,$fechainicio,$fechafinal,$tablename,$cantidad,$initialport,$finalport,&$sql){
    $i=0;
    $portinitemp = $initialport;
    $portfintemp = $finalport;
    ...
    for ($i=0;$i<count($dates);$i++){
        ...
        $left_percent = $GLOBALS["conexion"]->executeQuery("SET @left_percent = @left_percent - @percent_per_day;");
        $left_percent = $GLOBALS["conexion"]->executeQuery("SELECT @left_percent;");
        $left_percent = $left_percent->get("@left_percent");
          //echo $sql2;    
    }
...
if($_GET['ajax']==22){  
    ...
    $title=" Ocupacionales del Gateway ".$gateway;
    $worksheet->write(2,0,$title,$title_format);
...
*/        
        $query = "";
        //$data = ocupacion($fechainicio,$fechafinal,$gateway,$cantidad,$initialPort,$finalPort,$query);
        //$data = $conex->FetchAll($data);
        //$data = ocupacion2($dates,$fechainicio,$fechafinal,$gateway,$cantidad,$initialPort,$finalPort,&$query);
        $data = ocupacion3($dates,$fechainicio,$fechafinal,$gateway,$cantidad,$initialPort,$finalPort,$query);
        $data = $conex->FetchAll($query);
        $conex->Close();
        $conex = new FetchUtil($dsn);                   
        ...
        <table width="99%" border="0" align="center" cellpadding="0" cellspacing="0" class="tableBorder">
          <tr>
            <td colspan="8" class="lineHorizontal lineVerticalD"><div align="center"><span class="styleNBLM">Gateway:&nbsp;<?=$gateway;?>&nbsp;&nbsp;&nbsp;puertos&nbsp;del&nbsp;<?=$initialPort?>&nbsp;al&nbsp;<?=$finalPort;?>&nbsp;&nbsp;&nbsp;Del&nbsp;periodo&nbsp;<?=$fechainicio;?>&nbsp;a&nbsp;<?=$fechafinal;?>&nbsp;&nbsp;&nbsp;de&nbsp;un&nbsp;total&nbsp;de&nbsp;<?=$cantidad;?>&nbsp;minutos</span></div></td>
          </tr>
...
<!--                <tr>
                    <td colspan="8" class="lineHorizontal lineVerticalD">&nbsp;</td>
                    <td colspan="8" class="lineVerticalD">&nbsp;</td>
                </tr>
-->
                <tr onmouseover="this.style.backgroundColor='#CCFFCC'; this.style.color='#000000'"
                onmouseout="this.style.backgroundColor='#FFFFFF';this.style.color='#000000'" 
    ...
    <!--                    <td class="lineHorizontal lineVerticalD"><div align="center"><span class="styleContent"><?//=$dates[$j];?>&nbsp;<?//=$data->get($dates[$j]);?>&nbsp;</span></div></td> -->
                            <td class="lineHorizontal lineVerticalD"width="100%" onmouseover="this.style.backgroundColor='#CCFFCC'; this.style.color='#000000'"
                                onmouseout="this.style.backgroundColor='#E0E0E0';this.style.color='#000000'"
                                style="cursor:default; background-color:#E0E0E0; color='#000000'">
                                <div align="center"><span class="styleNBLM"><?=$dates[$j];?></span></div>
                                <?
                                $ocupacional="Gateway: $gateway  puertos del $initialPort al $finalPort  Del periodo $fechainicio a $fechafinal de un total de $cantidad minutos";
                                $worksheet->write($b=($i+1)*4,0,$ocupacional,$title_format);
                                $worksheet->write($b+1,$j,$dates[$j],$cabecera);
                                ?>
                            </td>
                        </tr>
                        <tr>
                            <td class="lineHorizontal lineVerticalD"width="100%"onmouseover="this.style.backgroundColor='#CCFFCC'; this.style.color='#000000'"
                                onmouseout="this.style.backgroundColor='#FFFFFF';this.style.color='#000000'" 
                                style="cursor:default; background-color:#FFFFFF; color='#000000'">
                                <div align="center"><span class="styleContent"><?=$data[0][$dates[$j]];?>&nbsp;%</span></div>
                                <?$worksheet->write($b+2,$j,$data[0][$dates[$j]]." %" ,$texto);?>
                            </td>
                        </tr>
                    </table>    
                </td> 
            <?
            $j++;    
            if($j==(sizeof($dates)) && sizeof($dates)<8){
                for($k=(sizeof($dates));$k<8;$k++){
                    $conex->Close();
                    $conex = new FetchUtil($dsn);
                    ?>     
                    <td width="<?=100/8?>%" onmouseover="this.style.backgroundColor='#FFFFFF'; this.style.color='#000000'"
                onmouseout="this.style.backgroundColor='#FFFFFF';this.style.color='#000000'" 
                style="cursor:default; background-color:#FFFFFF; color='#000000'"> 
                        <table width="100%" border="0" cellpadding="0" cellspacing="0">
                            <tr>
                                <td class="lineHorizontal lineVerticalD" width="100%"><div align="center"><span class="styleNBLM"></span></div></td>
                            </tr>
                            <tr>
                                <td class="lineHorizontal lineVerticalD" width="100%"><div align="center"><span class="styleContent"></span></div></td>
                            </tr>
                        </table>    
                    </td> 
                    <?
